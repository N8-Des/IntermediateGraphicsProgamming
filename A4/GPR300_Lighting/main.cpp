...
int main() {
	...
	//lmao I should re-organize this but I won't 
	GLuint textureRock;
	...
	unsigned char* textureData = stbi_load("Stone.png", &width, &height, &numComponents, 0);
	...
	stbi_set_flip_vertically_on_load(true);
	textureData = stbi_load("Bark.png", &width, &height, &numComponents, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, textureData);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	GLuint textureRockNormal;
	glGenTextures(1, &textureRockNormal);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, textureRockNormal);
	stbi_set_flip_vertically_on_load(true);
	textureData = stbi_load("StoneNormal.png", &width, &height, &numComponents, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, textureData);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	GLuint textureBarkNormal;
	glGenTextures(1, &textureBarkNormal);
	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, textureBarkNormal);
	stbi_set_flip_vertically_on_load(true);
	textureData = stbi_load("StoneNormal.png", &width, &height, &numComponents, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, textureData);
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		litShader.setFloat("_NormalStrength", normalStrength);
		...
		ImGui::SliderFloat("Normal Strength", &normalStrength, 0, 10);
