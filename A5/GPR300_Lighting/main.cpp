...
bool postProcessingCheck = false;
int postProcessingEffect = 1;
int usingPostProcessing = 0;
...
int main() {
	...
	//its me the post processing shader
	Shader postProcessingShader("shaders/PostProcessing.vert", "shaders/PostProcessing.frag");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, fbTex);
		postProcessingShader.setInt("_UseEffect", usingPostProcessing);
		postProcessingShader.setInt("_RenderedTex", 1);
		postProcessingShader.setInt("_SelectedEffect", postProcessingEffect);
		postProcessingShader.setInt("_Width", SCREEN_WIDTH);
		postProcessingShader.setInt("_Height", SCREEN_HEIGHT);
		postProcessingShader.setFloat("_Time", time);
		quadMesh.draw();
		...
		ImGui::Checkbox("Toggle Post Processing", &postProcessingCheck);
		ImGui::SliderInt("Current Effect", &postProcessingEffect, 1, 3);
		ImGui::End();

		if (postProcessingCheck)
		{
			usingPostProcessing = 1;
		}
		else
		{
			usingPostProcessing = 0;
		}
